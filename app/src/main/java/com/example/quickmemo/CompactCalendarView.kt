...
class CompactCalendarView @JvmOverloads constructor(
    ...
    var onDateTap: ((year: Int, month: Int, day: Int) -> Unit)? = null
    ...
    fun refreshHolidays() { invalidate() }
    ...
    private fun drawHeader(canvas: Canvas, w: Float) {
        val headerText = if (Locale.getDefault().language == "ja") {
            ...
            "${m[displayMonth]} $displayYear"
        }
        textPaint.apply { color = 0xFFDDDDDD.toInt(); textSize = cellH * 0.55f }
    ...
    private fun drawWeekdays(canvas: Canvas) {
        ...
        for (i in 0..6) {
            textPaint.color = when (i) {
                0 -> 0xFFE05050.toInt(); 6 -> 0xFF5599CC.toInt()
    ...
    private fun drawDays(canvas: Canvas) {
        ...
        val isCurrentMonth = displayYear == today.get(Calendar.YEAR) && displayMonth == today.get(Calendar.MONTH)
        ...
        for (day in 1..daysInMonth) {
            ...
            if (isCurrentMonth && day == todayDate) {
                ...
                canvas.drawCircle(cx, cy, cellH * 0.3f, circlePaint)
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_UP) {
            ...
            if (leftArrowRect.contains(x, y)) { prevMonth(); return true }
            if (rightArrowRect.contains(x, y)) { nextMonth(); return true }

            // Check if a day cell was tapped
            val baseY = headerHeight + cellH
            if (y > baseY) {
                val col = (x / cellW).toInt()
                val row = ((y - baseY) / cellH).toInt()
                val firstDow = getFirstDayOfWeek(displayYear, displayMonth)
                val dayIndex = row * 7 + col - firstDow + 1
                val daysInMonth = getDaysInMonth(displayYear, displayMonth)
                if (dayIndex in 1..daysInMonth) {
                    onDateTap?.invoke(displayYear, displayMonth, dayIndex)
                }
            }
        }
        return true
    ...
    private fun getDaysInMonth(year: Int, month: Int): Int {
        val c = Calendar.getInstance(); c.set(year, month, 1)
